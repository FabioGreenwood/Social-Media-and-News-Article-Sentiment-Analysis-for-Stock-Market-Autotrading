...
design_space_dict = {
    "senti_inputs_params_dict" : {
        "topic_qty" : [0]#, # 0 - nether
        #"topic_model_alpha" : [0.3, 0.7, 1, 2, 3, 5, 7, 13], # 1 - nether
        #"relative_halflife" : [3*60, 0.25 * SECS_IN_AN_HOUR, 2*SECS_IN_AN_HOUR, 7*SECS_IN_AN_HOUR], # 2 - sentiment only
        #"apply_IDF" : [False, True],  # 3 - sentiment only
        #"topic_weight_square_factor" : [1, 2, 4], # 4 - sentiment only
        #"factor_tweet_attention" : [False, True], # 5 - sentiment only
        #"factor_topic_volume" : {0 : False, 1 : True, 2 : global_exclusively_str} # 6  - sentiment only
    },
    "model_hyper_params" : {
