...
default_temporal_params_dict        = {
    ...
    "train_period_end"      : datetime.strptime('01/03/16 00:00:00', global_strptime_str), # placeholder
...
def update_global_record(pred_steps, df_designs_record, experi_params_list, run_name, global_record_path):
    ...
    try:
        ...
        df_global_record_t.to_csv(global_record_path + "T")
    except:
        df_global_record.to_csv(global_record_path[:-4] + "T.csv")    
...
init_doe = 35
init_doe = [
[1,	12,	2,      720,   0,	2,	4,	3,	1.01E-09,	10, 3, 1, 1, 2, 0.001, 0], #baseline old activiation back
[2,	9,	2,      720,   0,	2,	4,	3,	1.01E-09,	10, 3, 40, 1, 2, 0.001, 0], #baseline old activiation back
#1[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 0, 60, 1, 2, 0.001, 0], #more epochs
#1[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 3, 60, 1, 1, 0.001, 0], #train with MSE loss function
#2[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 3, 60, 3, 2, 0.001, 0], #more ensamble models
#3[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 3, 70, 1, 1, 0.0005, 0], #decreased learning rate
#4[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 4, 70, 1, 1, 0.00001, 0], #decreased learning rate 2
#5[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 3, 40, 1, 1, 0.001, 1], #softmax activation
#5[2,	9,	1,      720,   0,	2,	4,	3,	1.00E-09,	10, 7, 50, 1, 2, 0.001, 0], #more patentice
]
...
for scenario_ID in [3, 4, 5]:
    ...
    scenario_name_str = scenario_name_str + "performance_test_{}.csv".format(str(shard))
    ...
    if __name__ == '__main__':
        ...
        experiment_manager(
            ...
            global_record_path=os.path.join(global_general_folder,r"outputs/performance_test_{}.csv".format(str(shard)))
            )
