...
def compare_design_dict_to_df(design_params_list, df, design_space_dict=design_space_dict, mode=None):
    ...
    if len(design) > 5:
        ...
        if mode == "explict":
            ...
            correct_rows    = (matching_values - len(df[design.index].columns) > -3)
        else:
...
def read_csvs_in_folder(folder_path, file_suffix=".csv"):
    csv_files = [file for file in os.listdir(folder_path) if file.endswith(file_suffix) and not file.endswith("T" + file_suffix)]
    dataframes = {}
...
for design_ID in range(len(list_of_design_inputs)):
    ...
    count_implicit = 0
    history = []
    for df in list_dataframes.values():
        count_explict  += compare_design_dict_to_df(list_of_design_inputs[design_ID], df, mode = "explict")
        count_implicit += compare_design_dict_to_df(list_of_design_inputs[design_ID], df, mode = "implict")
    print("ID:{} explict:{} implicit:{}".format(design_ID, count_explict, count_implicit))