...
colors = ["blue", "violet", "pink", 'darkgoldenrod', 'darkgreen', 'red', 'darkturquoise', "violet", 'indianred', 'yellowgreen']
...
shards_studied = [3, 9]
plot_assignment = { # 0: high, 1: mid, 2: low, None:excluded
    3 : {5 : 0, 14 : 0, 17 : 0, 12 : 1, 8  : 2, 18 : 2},
    9 : {11 : 0, 14 : 0, 17 : 0, 7  : 1, 5  : 2, 12 : 2, 16 : 2, 8 : None, 18 : None}}
...
for s in shards_studied:
    ...
    for row in df_param_data.index:
        ...
        data_set_new = {
            "shard" : s,
            "local ID" : local_ID,
            "series_name" : "local ID: {}".format(local_ID, return_model_type_name(df_param_data.loc[row, param_cols_sans_temp_list])),
    ...
    for i, data_set in enumerate(data_set_list_single_shard):
        plot_asignment = plot_assignment[s][data_set["local ID"]]
        if plot_asignment != None:
            x_values          = list(data_set["different_temporal_values"].keys())
            x_values.sort()
            x_values_str = [x.strftime("%Y-%m-%d") for x in x_values]
            ...
            axes[plot_asignment].plot(x_values_str, y_values_temporal, label = data_set["series_name"] + "testing score: " + original_scores_list[i], color=colors[i], linestyle="-") 
            axes[plot_asignment].plot(x_values_str, y_values_temporal_mean, color=colors[i], linestyle=":")
            axes[plot_asignment].plot(x_values_str, y_values_original, color=colors[i], linestyle="--") 
            axes[plot_asignment].set_xticks(range(len(x_values_str)))
            axes[plot_asignment].set_xticklabels([f'{i}' for i in x_values_str], rotation='vertical')
            y_low  = min(y_low , min(y_values_temporal))
            y_high = max(y_high, max(y_values_temporal))
    
    axes[0].set_title("{} Validation Score Designs".format("High"))
    axes[1].set_title("{} Validation Score Designs".format("Mid"))
    axes[2].set_title("{} Validation Score Designs".format("Low"))
    for j in range(3):
        axes[j].set_ylim((y_low,y_high))

    
    #axes[0].set_xticklabels([f'{i}' for i in x_values], rotation='vertical')
    #axes[0].set_xlabel(temporal_label_dict_chart_label_str, labelpad=20)
    #axes[0].set_ylabel('testing score', labelpad=20)
    ...
    fig.suptitle("Testing Profitability Per Month ({} Min Prediction Horizon)".format(str(int(5 * param_data["pred_steps"]))))
    ...
    fig.suptitle("Overall Title for the Plot")
...
for data_set_single_shard in data_set_list:
    for entry in data_set_single_shard:
        df_temporal_results_csv.loc[row, "shard"] = entry["shard"]
